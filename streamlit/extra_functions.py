...
def compare_chambers(conn):
    ...
    st.html("<h2>Chambers Comparison")

    # Display Judgment Count Comparison in a separate row
    with st.container():
        st.html("<h1>Judgment Count Comparison")
    ...
    with st.container():
        st.html("<h1>Criminal Judgment Comparison")
    ...
    with st.container():
        st.html("<h1>Civil Judgment Comparison")
...
def display_judgments_by_judge(conn):
    ...
    if df.empty:
        st.html("<p>No data available for judgments by judge.")
        return

    df["Judge"] = df["Judge"].str.title()

    st.html("<h1>Judgments by Judge")
...
def display_number_of_judgments_by_chamber(conn):
    ...
    st.html("<h1>Judgments by Chamber")
